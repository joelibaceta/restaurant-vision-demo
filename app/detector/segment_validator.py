# detector/segment_validator.py — Validación de segmentos con YOLO
import numpy as np


class SegmentValidator:
    """Valida que los segmentos cortados contengan personas válidas con cabeza"""
    ...
    def validate_person_segment(self, frame, bbox):
        """
        Validación secundaria con YOLO del segmento cortado.
        Verifica que el segmento contenga al menos la CABEZA de una persona.
        Para personas sentadas, no necesita estar completa, pero SÍ debe tener cabeza.
        """
        try:
            x1, y1, x2, y2 = bbox
            h, w = frame.shape[:2]
            
            # Validar que el bbox esté dentro del frame
            x1, y1, x2, y2 = max(0, int(x1)), max(0, int(y1)), min(w, int(x2)), min(h, int(y2))
            
            if x2 <= x1 or y2 <= y1:
                return False
                
            # Extraer el segmento con padding
            segment = self._extract_segment_with_padding(frame, x1, y1, x2, y2)
            if segment is None:
                return False
            
            # Ejecutar YOLO en el segmento
            best_detection = self._find_best_detection_in_segment(segment)
            if best_detection is None:
                return False
            
            # Validar la detección encontrada
    ...
    def _extract_segment_with_padding(self, frame, x1, y1, x2, y2):
        """Extrae el segmento con padding para contexto"""
        padding = 15
        h, w = frame.shape[:2]
    ...
    def _find_best_detection_in_segment(self, segment):
        """Encuentra la mejor detección de persona en el segmento"""
        segment_results = self.model.predict(
            segment, 
            conf=self.conf * 0.8,  # Confianza más permisiva
        ...
        for (sx1, sy1, sx2, sy2), conf in zip(xyxy, confs):
            ...
            # La detección debe ocupar una porción razonable del segmento
            area_ratio = seg_area / segment_area
            
            if area_ratio > 0.2 and conf > best_conf:  # Al menos 20% del segmento
    ...
    def _validate_detection_quality(self, segment, detection):
        """Valida la calidad de la detección encontrada"""
        sx1, sy1, sx2, sy2 = detection
        seg_w, seg_h = sx2 - sx1, sy2 - sy1
        seg_aspect_ratio = seg_h / max(seg_w, 1e-6)
        
        # Filtros más permisivos para personas sentadas
        
        # 1. Aspect ratio más permisivo
        if seg_aspect_ratio < 0.8:
            return False
            
        # 2. Tamaño mínimo más permisivo
        if seg_h < 30 or seg_w < 15:
            return False
        
        # 3. Si hay YOLO Pose disponible, ENFOCARSE EN LA CABEZA
        ...
        # Si llegamos aquí sin YOLO Pose, aceptar si tiene forma razonable
        return True
    
    def _validate_segment_has_head(self, segment):
        """
        Validación enfocada en detectar CABEZA en el segmento.
        Para personas sentadas, solo necesitamos confirmar que hay una cabeza.
        """
        try:
            pose_results = self.pose_model.predict(
                segment, 
                conf=self.conf * 0.4,  # Confianza más baja para detectar cabezas parciales
                verbose=False
            )[0]
            ...
            for person_kpts, person_confs in zip(keypoints, confidences):
                # Keypoints específicos de la CABEZA
                head_kpts = [0, 1, 2, 3, 4]  # nariz, ojo_izq, ojo_der, oreja_izq, oreja_der
                torso_kpts = [5, 6]  # hombros (parte alta del torso)
                ...
                # CRITERIO PRINCIPAL: DEBE tener cabeza O parte alta del torso
                has_head = head_count >= 1  # Al menos 1 punto de la cabeza
                has_upper_torso = torso_count >= 1  # Al menos 1 hombro
                
                # RECHAZAR si SOLO detecta extremidades inferiores
                only_lower_body = lower_count > 0 and head_count == 0 and torso_count == 0
                if only_lower_body:
                    return False
                
                # ACEPTAR si tiene cabeza o parte alta del torso
